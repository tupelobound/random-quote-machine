...
function getQuote () {
  ...
  $.getJSON('https://api.forismatic.com/api/1.0/?method=getQuote&format=jsonp&lang=en&jsonp=?', function (json) {
    let quote = json.quoteText // set variable for the quote
    let author = json.quoteAuthor // set variable for the quote's author
    ...
    const red = Math.floor(Math.random() * 170)
    const green = Math.floor(Math.random() * 170)
    const blue = Math.floor(Math.random() * 170)
    const mix = `rgb(${red}, ${green}, ${blue})` // create a string variable for a random rgb color
